...
PROMPT = """\
以下の複数の画像を読み取り、**画像に写っている全ての名刺** から個人情報を抽出して JSON 配列で返してください。

重要:
- 1 枚の画像に **複数枚の名刺が写っている場合は、全部を別要素として抽出** すること（並べて撮影した名刺、スキャンした複数枚、机の上に並んだ名刺など）
- 逆に 1 枚の名刺が複数の画像に分かれて写っている場合は 1 要素にまとめること
- 抽出結果の件数は画像枚数と一致しなくてよい（画像 4 枚から 26 枚の名刺が出ることも普通にありうる）
